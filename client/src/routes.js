...
import { updateStateToCampaignInfo } from './redux/actions/activeCampaign';

export default store => {
  const updateCurrentCampaign = (id, callback) => {
    store.dispatch(updateStateToCampaignInfo(id)).then(callback);
  };
    ...
    <Route path="/" getComponent={(location, callback) => callback(null, App)}>
      ...
      <Route
        ...
        getComponent={(location, callback) => {
          updateCurrentCampaign(location.params.id, callback(null, CampaignPage));
        }}
      />
      <Route
